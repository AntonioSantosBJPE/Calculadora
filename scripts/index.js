...
function createEventsExtraButtons() {
  ...
  btnDel.addEventListener("click", (event) => {
    ...
    arrTemp.pop();
    display.innerHTML = arrTemp.join("");
  });
  ...
  btnEqual.addEventListener("click", (event) => {
    ...
    if (!varGlobal.validationEqual) {
      if (
        varGlobal.previousNumberText[
          varGlobal.previousNumberText.length - 1
        ] === "+"
      ) {
        varGlobal.resultCalc =
          varGlobal.previousNumber + varGlobal.currentNumber;
